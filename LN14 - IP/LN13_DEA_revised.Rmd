...
## 3) CCR 및 BCC 분석
```{r}
te <- dea(dea_set, noutput = 1, orientation = 1, rts = 1) # CCR Model / CRS(불변규모수익)
pte <- dea(dea_set, noutput = 1, orientation = 1, rts = 2) # BCC Model / VRS(가변규모수익)
```
...
가맹본부가 운영하는 총 매장 수, 임직원 수, 가맹점주에게 부과하는 비용 이 
가맹본부의 매출을 얼마나 발생시키는가? 

투입 : 매장 수(물적자원), 임직원 수(인적자원), 가맹점주가 부담하는 비용(가맹본부 영업역량)
산출 : 가맹본부 매출액(기업의 성과)
...
coffee_result <-
  coffee_data %>% 
  mutate(fyear = as.numeric((mdy("12/6/2021") - mdy(fran_start_year))/365)) %>% 
  select(DMU, company, brand, com_type, n_brand, emp_total, 
         fyear, total_store_2020, total_fee, te, pte, scale_eff_cate, benchmark)
```
...
tobit_model <- 
  censReg(pte ~ com_type + n_brand + emp_total + fyear + total_store_2020 + total_fee, 
          left = lower_bound, 
          right = upper_bound, 
          data = coffee_result)
```
